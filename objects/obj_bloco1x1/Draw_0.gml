...
if (fogo.alp_fogoso > 0){
    ...
    draw_sprite_ext(sprite_index, image_index, x, y, xscale, yscale, angle, #FFAE00, fogo.alp_fogoso/4);
    shader_reset();
}
...
if (predio != noone and sprite_index != spr_predio_bege and sprite_index != spr_predio_cinza and sprite_index != spr_predio_roxo){
    ...
    if (fogo.alp_fogoso > 0){
        shader_set(sh_multi_color);
        draw_sprite_ext(predio, num_predio, x, y-(sprite_height*yscale)/2, xscale, yscale, angle, c_orange, fogo.alp_fogoso/4);
        shader_reset();
    }
